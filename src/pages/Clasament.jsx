...
function Clasament() {
  ...
  useEffect(() => {
    const initClasament = async () => {
      try {
        ...
        const [xpSnap, probSnap] = await Promise.all([getDocs(xpQ), getDocs(probQ)]);
        ...
        setUltimulDocXP(xpSnap.docs[xpSnap.docs.length - 1]);
        setUltimulDocProbleme(probSnap.docs[probSnap.docs.length - 1]);
      } catch (err) {
        console.error(err);
      }
      setLoading(false);
  ...
  // "Înapoi" nu mai face fetch, doar decrementează pagina din date deja încărcate
  const paginaInapoiXP = () => {
    if (paginaXP > 1) setPaginaXP((prev) => prev - 1);
  };

  const paginaInapoiProbleme = () => {
    if (paginaProbleme > 1) setPaginaProbleme((prev) => prev - 1);
  };
  ...
  // Pagina "Înainte" e disponibilă dacă pagina curentă e ultima din datele încărcate
  // (mai pot exista date în Firebase neîncărcate încă)
      ...
      <div className="topuri-container">
        <section className="top-section">
          <div className="leaderboard-card">
            ...
            <div className="pagination">
              <button className="page-btn" disabled={paginaXP === 1} onClick={paginaInapoiXP}>
                Înapoi
              </button>
              <span className="page-info">Pagina {paginaXP}</span>
              <button className="page-btn" disabled={!areMoreXP} onClick={paginaUrmatoareXP}>
                Înainte
              </button>
            </div>
          </div>
        ...
        <section className="top-section">
          <div className="leaderboard-card">
            ...
            <div className="pagination">
              <button className="page-btn" disabled={paginaProbleme === 1} onClick={paginaInapoiProbleme}>
                Înapoi
              </button>
              <span className="page-info">Pagina {paginaProbleme}</span>
              <button className="page-btn" disabled={!areMoreProbleme} onClick={paginaUrmatoareProbleme}>
                Înainte
              </button>
            </div>
          </div>
